...
const TEMPLATE_HEADERS = ["Name", "Class", "School", "Parent Phone", "Total Fee", "Paid", "Due Date"];
...
function parseWorkbook(arrayBuffer) {
  ...
  return rows.map((row, i) => {
    ...
    const total = Number(map["totalfee"] || map["total"] || 0);
    const paid = Number(map["paid"] || 0);
    const due = excelDateToISO(map["duedate"] || map["due"]);
    const errors = [];
    if (!name) errors.push("Missing name");
    if (!total || total <= 0) errors.push("Missing/invalid total fee");
    if (!due) errors.push("Missing/invalid due date");
    return { rowNum: i + 2, id: "tmp" + i, name, cls: cls || "—", school: school || SCHOOLS[0], phone, total, paid: paid || 0, due, errors };
  });
}
...
function FeeLedger({ user, onLogout }) {
  ...
  const [newStudent, setNewStudent] = useState({ name: "", cls: "", school: SCHOOLS[0], phone: "", total: "", paid: "", due: "" });
  ...
  const stats = useMemo(() => {
    const pool = students.map((s) => ({ ...s, status: statusOf(s) })).filter((s) => (schoolFilter === "All Schools" ? true : s.school === schoolFilter));
    return {
      count: pool.length,
  ...
  async function saveStudent() {
    if (!newStudent.name || !newStudent.total || !newStudent.due) {
      return setToast({ kind: "warn", text: "Name, total fee, and due date are required." });
    }
    try {
      if (editingId) {
        const updated = await api.updateStudent(editingId, newStudent);
        setStudents((prev) => prev.map((s) => (s.id === editingId ? updated : s)));
        setToast({ kind: "ok", text: "Student updated." });
      } else {
        const created = await api.addStudent(newStudent);
        setStudents((prev) => [created, ...prev]);
        setToast({ kind: "ok", text: "Student added." });
      }
      setShowAdd(false);
      setEditingId(null);
      setNewStudent({ name: "", cls: "", school: SCHOOLS[0], phone: "", total: "", paid: "", due: "" });
    } catch (err) {
      setToast({ kind: "warn", text: err.message });
    }
  }
  ...
  function openEdit(s) {
    setEditingId(s.id);
    setNewStudent({ name: s.name, cls: s.cls, school: s.school, phone: s.phone, total: String(s.total), paid: String(s.paid), due: s.due });
    setShowAdd(true);
  }
  ...
  async function confirmImport() {
    const valid = importRows.filter((r) => r.name && r.total > 0 && r.due).map(({ errors, rowNum, id, ...s }) => s);
            ...
            return (
              <div className="row" key={s.id}>
                ...
                <div className="row-right">
                  <div className={`amounts ${(s.payments || []).length ? "clickable" : ""}`} onClick={() => setHistoryStudent(s)}>
                  ...
                  <div className="row-actions">
                    ...
                    {balance > 0 && <button className="icon-btn" title="Record payment" onClick={() => setPayModal(s)}><IndianRupee size={14} /></button>}
