...
app.post("/api/students/bulk-import", requireAuth, requireAdmin, h(async (req, res) => {
  ...
  const prepared = students
    ...
    .map((r) => ({
      ...
      annualFeeAmount: Number(r.annualFeeAmount) || 0,
...
// Annual Fee is a single once-per-academic-year charge (see db.js) — same
...
app.post("/api/students/:id/annual-fee/payments", requireAuth, h(async (req, res) => {
  ...
  if (!amount || amount <= 0) return res.status(400).json({ error: "amount must be a positive number" });
  const method = req.body.method === "upi_bank" ? "upi_bank" : "cash";
  const updated = await db.addAnnualFeePayment(req.params.id, amount, method, req.user.name || req.user.email);
  if (!updated) return res.status(404).json({ error: "Student not found" });
  res.json(updated);
}));
