...
import { Search, Plus, MessageCircle, X, Check, Clock, AlertTriangle, IndianRupee, Send, History, Trash2, Upload, Download, FileSpreadsheet, AlertCircle, Pencil, LogOut, ChevronDown } from "lucide-react";
...
function FeeLedger({ user, onLogout }) {
  ...
  const [newStudent, setNewStudent] = useState({ name: "", cls: "", school: SCHOOLS[0], phone: "", total: "", paid: "", due: "", planSelect: "full", installmentAmount: "" });
  ...
  async function saveStudent() {
    ...
    try {
      ...
      setShowAdd(false);
      setEditingId(null);
      setNewStudent({ name: "", cls: "", school: SCHOOLS[0], phone: "", total: "", paid: "", due: "", planSelect: "full", installmentAmount: "" });
    } catch (err) {
      setToast({ kind: "warn", text: err.message });
    }
  }
  ...
  function handleFileSelect(e) {
    ...
    reader.onload = (evt) => {
      try {
        setImportRows(parseWorkbook(evt.target.result));
      } catch {
        ...
        <div className="modal-backdrop" onClick={() => { setShowAdd(false); setEditingId(null); }}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="field-row">
              ...
              <div className="field"><label>School</label><select value={newStudent.school} onChange={(e) => setNewStudent({ ...newStudent, school: e.target.value })}>{SCHOOLS.map((s) => <option key={s}>{s}</option>)}</select></div>
            </div>
            ...
            {importRows && (
              <>
                ...
                <div style={{ maxHeight: 320, overflow: "auto", marginBottom: 12 }}>
                  {importRows.map((r) => (
                    <div key={r.id} className="reminder-item" style={{ background: r.errors.length ? "#FBEAE8" : "#fff" }}>
